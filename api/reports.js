...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (req.method === 'POST') {
            const { Date, Area, Team, Tag, Code, Category, Description, Unit, Qty, PipeLength, Manpower, Manhours, Foreign, Remark, CreatedBy } = req.body;
            ...
            const result = await neon(
                `INSERT INTO reports (date, area, team, tag, code, category, description, unit, qty, pipe_length, manpower, manhours, foreign_count, remark, created_by, created_at)
                 ...
                 RETURNING id`,
                [Date, Area, Team, Tag || 'N/A', Code || '', Category || '', Description || '', Unit || '', Qty || 0, PipeLength || 0, Manpower || 0, Manhours || 0, Foreign || 0, Remark || '', CreatedBy || 'unknown']
            );
